...
@section('content')
    <div class="p-6">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900">
                        <span class="text-blue-600">✏️</span> Editar Servicio
                    </h1>
                    <p class="text-gray-600 mt-1">Actualiza la información de tu servicio</p>
                </div>
                <div class="flex space-x-3">
                    <a href="{{ route('supplier.services.show', $service) }}"
        ...
        <form action="{{ route('supplier.services.update', $service) }}" method="POST" enctype="multipart/form-data"
            class="space-y-8">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="lg:col-span-2">
                        ...
                        <textarea id="description" name="description" rows="4" required
                            class="block w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200"
                            placeholder="Describe qué incluye tu paquete, materiales, tiempo de entrega, y cualquier restricción.">{{ old('description', $service->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                                ...
                                <div class="flex items-center space-x-4">
                                    <label class="flex items-center">
                                        <input type="checkbox" id="urgent_available" name="urgent_available" value="1"
                                            {{ old('urgent_available', $service->urgent_available) ? 'checked' : '' }}
                                            class="h-4 w-4 text-red-600 focus:ring-red-500 border-gray-300 rounded">
                                        <span class="ml-2 text-sm text-red-700">
                                            Permite a los clientes cotizar este servicio con poco tiempo de anticipación,
                                            aplicando un costo mayor.
                                        </span>
                                    </label>
                                </div>

                                <div id="urgent_price_section"
                                    class="mt-4 {{ old('urgent_available', $service->urgent_available) ? '' : 'hidden' }}">
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="number" id="urgent_price_extra" name="urgent_price_extra"
                                            value="{{ old('urgent_price_extra', $service->urgent_price_extra) }}"
                                            step="0.01" min="0"
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                @if ($service->main_image)
                    <div class="mb-6">
                        ...
                        <div class="flex items-center space-x-4">
                            <img src="{{ $service->getMainImageUrl() }}" alt="Imagen principal actual"
                                class="w-32 h-32 object-cover rounded-lg border border-gray-200">
                            <div>
                                <p class="text-sm text-gray-600">Esta es tu imagen principal actual.</p>
                ...
                <div class="mb-6">
                    <label for="main_image" class="block text-sm font-medium text-gray-700 mb-2">
                        {{ $service->main_image ? 'Nueva Imagen Principal (Opcional)' : 'Imagen Principal del Servicio' }}
                    </label>
                    <div
                        ...
                        <div class="space-y-1 text-center">
                            ...
                            <div class="flex text-sm text-gray-600">
                                <label for="main_image"
                                    ...
                                    <span>{{ $service->main_image ? 'Cambiar imagen principal' : 'Subir imagen principal' }}</span>
                ...
                @if ($service->portfolio_images && count($service->portfolio_images) > 0)
                    <div class="mb-6">
                        <label class="block text-sm font-medium text-gray-700 mb-2">Portafolio Actual</label>
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                            @foreach ($service->getPortfolioImagesUrls() as $imageUrl)
                                <div class="relative">
                                    <img src="{{ $imageUrl }}" alt="Imagen del portafolio"
                                        class="w-full h-24 object-cover rounded-lg border border-gray-200">
                                </div>
                            @endforeach
                        </div>
                        <p class="text-xs text-gray-500 mt-2">Sube nuevas imágenes para reemplazar todo el portafolio
                            actual.</p>
                    </div>
                @endif

                <!-- Nuevas Fotos de Referencia -->
                <div>
                    <label for="portfolio_images" class="block text-sm font-medium text-gray-700 mb-2">
                        {{ $service->portfolio_images ? 'Nuevo Portafolio (Opcional)' : 'Subir Fotos de Referencia' }}
                    </label>
                    <div
                        class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg hover:border-gray-400 transition-colors">
                        <div class="space-y-1 text-center">
                            ...
                            <div class="flex text-sm text-gray-600">
                                <label for="portfolio_images"
                                    class="relative cursor-pointer bg-white rounded-md font-medium text-blue-600 hover:text-blue-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-blue-500">
                                    <span>{{ $service->portfolio_images ? 'Cambiar portafolio' : 'Sube tus archivos' }}</span>
                                    <input id="portfolio_images" name="portfolio_images[]" type="file"
                                        class="sr-only" accept="image/*" multiple>
                                </label>
                                <p class="pl-1">o arrastra y suelta</p>
                            </div>
                            <p class="text-xs text-gray-500">PNG, JPG, GIF hasta 5MB cada una</p>
                        </div>
                    </div>
